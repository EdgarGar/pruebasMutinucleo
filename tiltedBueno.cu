...
_global_ void multMatrixOnGPU2D(int *MatA, int *MatB, int *MatC, int nx, int ny)
{
...
_global_ void tiledMult(int *MatA, int *MatB, int *MatC, int nx, int ny)
{
    unsigned int ix = threadIdx.x + blockIdx.x * blockDim.x;
    unsigned int iy = threadIdx.y + blockIdx.y * blockDim.y;
    // unsigned int idx = iy * nx + ix;

    _shared_ float sharedMatA[TILEDIM][TILEDIM];
    _shared_ float sharedMatB[TILEDIM][TILEDIM];
